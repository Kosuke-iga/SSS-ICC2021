...
That scheme can detect a RAP in stable traffic environment by revealing the channel used with a LAP with intentional interference.
...
In order to realize the user side detection which is independent of network environment, in this paper, we propose RAP detection by using Address Resolution Protocol (ARP) failure under the MAC address duplication.
...
On the basis of this idea, our scheme reveals a connected AP as a RAP by finding MAC address of such a LAP from obtainable beacon frames by a client \cite{beacon}.
...
Our scheme intentionally creates such situation by setting the MAC address of a client to that of the LAP on the other side of the connected RAP.
By doing this, our scheme can reveal the connected AP as a RAP regardless of network environment.
...
\begin{enumerate}
    ...
    \item The proposed scheme achieves accurate detection without any error even in real environment. It can guarantee beneficial effects on every available hotspot.
\end{enumerate}
...
By using RSSI, even if the MAC address is spoofed by adversary, that scheme can disclose that it is a RAP at different location from that of LAPs.
...
Thus, the adversary can evade the detection based packet delay by utilizing hardware-based RAPs having little bridging delay unlike software-based RAPs.
...
That scheme detects RAP by finding out the presence of these two distinct channels with the throughput of the transmission from the client to the DNS server.
...
The targeted network is open Wi-Fi, whose LAPs have no security protocols such as WPA2, in our model since MITM attack does not succeed in a protected Wi-Fi\cite{kataoka}.
%Thus, protected Wi-Fi is beyond the scope of our work.
By using same gateway with the LAN, the adversary can evade simple detections with Internet Services Provider (ISP) names or Global IP addresses \cite{rtt}.
...
In order to decrease the throughput, the previous scheme saturates the channel used between a LAP and the RAP by intentional interfering with an additional equipment in a client.
...
Thus, the requirement that we must satisfy is to leverage factors which are independent of the network environment.
...
By observing the continuance of resending ARP request packets within a definite period of time without ARP reply packets, the proposed scheme can reveal that there exists the RAP and LAP on the path, which detects the attack. %reply が来るまで送り続けます
...
%---------------------------------------------------------------------------------------------------------------SSIDの限定による候補の絞り込みについて（修正）
At this time, in addition, only the MAC addresses of APs which have the SSID used in the targeted network are collected since a RAP clones its SSID to deceive us.
By doing this, it can be more efficient since there exists several LAPs including irrelevant ones to the network in its communication range in a real situation.
%---------------------------------------------------------------------------------------------------------------SSIDの限定による候補の絞り込みについて（元々）
...
%---------------------------------------------------------------------------------------------------------------複数LAPの状況について（修正）
Furthermore, in the case where multiple LAPs which have same SSID and a RAP are set in the targeted LAN in real situations, the proposed scheme can detect without any problems.
In that case, only a client which forms MITM topology with a RAP cannot receive ARP replys if its MAC address is duplicated and the other can get them since they can be sent to both of the duplicated addresses.
Although it can not detect in the case a client does not form MITM with a RAP but it exists, the client does not have any risk to be eavesdropped on the exchange of its data.
...
By observing ARP packets in the situation, we can reveal the benignancy of connected AP.

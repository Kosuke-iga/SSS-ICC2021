...
First, in order to show the effectiveness of the proposed scheme, we compare it with the previous scheme \cite{previous} in terms of ACC.
...
Moreover, the lower FPR results in the higher ACC directly since the TPR is always 100.0\%.
...
Thus, in the case of less than 7.0 seconds, LAPs are detected as RAPs incorrectly since it is not enough for a user device to get ARP reply packets.
...
In order to demonstrate the effectiveness of the proposed scheme in a real environment, we conduct an experiment in a cafe and evaluate ACC and the total time required for the detection.
